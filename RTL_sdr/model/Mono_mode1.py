...
def convolve_compressed(x, h, expander, decimator):
    ouput_s = int((expander*x.size) / decimator)
    y = np.zeros(ouput_s)
    y_index = 0; x_index = 0
    i = 0
    while(i < x.size * 24):
...
def expand(x, expander):
	sample_expanded = np.zeros(x.size*expander)
	k = 0
	for i in range(x.size):
		sample_expanded[k] = x[i]
		k += expander
...
if __name__ == "__main__":
	...
	in_fname = "E:/mode1_sample_u8.raw"
	...
	audio_coeff = signal.firwin(audio_taps, \
		audio_Fc/(audio_Fs/2), window=('hann'))

	audio_coeff = audio_coeff * 24
	# set up the drawing
	...
	while (block_count+1)*block_size < len(iq_data):
		...
		#audio_block = convolve_compressed(fm_demod,audio_coeff,24,125)
		

		RF_front_end = expand(fm_demod,24)

		# extract the mono audio data through filtering
		audio_filt, state_lpf_16k = signal.lfilter(audio_coeff, 1.0, \
				RF_front_end, zi=state_lpf_16k)
		...
		if(block_count == 4):
			ax2.clear()
			ax2.psd(audio_data, NFFT=480*4, Fs=audio_Fs/(1e3*125))
		...
		if block_count >= 10 and block_count < 12:
			...
			ax0.clear()
			ax0.psd(fm_demod, NFFT=480, Fs=(rf_Fs/rf_decim)/1e3)
			...
			ax1.clear()
			ax1.psd(audio_filt, NFFT=480, Fs=(rf_Fs/rf_decim)/1e3)
			...
			ax2.clear()
			ax2.psd(audio_block, NFFT=480, Fs=audio_Fs/(1e3*125))
			ax2.set_ylabel('PSD (dB/Hz)')
			ax2.set_xlabel('Freq (kHz)')
			ax2.set_title('Mono Audio')
	...
	wavfile.write("../data/fmAudio.wav", int(48e3), np.int16((audio_data/2)*32767))
