...
def fast_Demod(I, Q, prev_phase=0.0):
    b = np.array([1, -1])
    a = np.array([1, 0])
    derI = signal.lfilter(b, a, I)
    derQ = signal.lfilter(b, a, Q)

    with np.errstate(divide='ignore'):
        fm_demod = np.true_divide(
            (I*derQ - Q*derI), (np.square(I)+np.square(Q)))
        fm_demod[fm_demod == np.inf] = 0
        fm_demod = np.nan_to_num(fm_demod)

    return fm_demod, math.atan2(Q[-1], I[-1])
